...
export default function AddressesPage() {
  ...
  const entries = db.entries.filter((e) => e.category === "address")
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt));

  return (
    <>
      <h2 style={{ marginBottom: 20 }}>Addresses / Places ({entries.length})</h2>
      {entries.map((entry) => (
      ...
      {entries.length === 0 && (
        <div className="recent-section">
          <p style={{ color: "var(--text2)" }}>No address entries yet.</p>
        </div>
      )}
    </>
  );
