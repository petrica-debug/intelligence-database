...
export default function VehiclesPage() {
  ...
  const entries = db.entries.filter((e) => e.category === "vehicle")
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt));

  return (
    <>
      <h2 style={{ marginBottom: 20 }}>Vehicles ({entries.length})</h2>
      {entries.map((entry) => (
      ...
      {entries.length === 0 && (
        <div className="recent-section">
          <p style={{ color: "var(--text2)" }}>No vehicle entries yet.</p>
        </div>
      )}
    </>
  );
