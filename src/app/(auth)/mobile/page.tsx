...
export default function MobilePage() {
  ...
  const entries = db.entries.filter((e) => e.category === "mobile")
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt));

  return (
    <>
      <h2 style={{ marginBottom: 20 }}>Mobile Numbers ({entries.length})</h2>
      {entries.map((entry) => (
      ...
      {entries.length === 0 && (
        <div className="recent-section">
          <p style={{ color: "var(--text2)" }}>No mobile number entries yet.</p>
        </div>
      )}
    </>
  );
